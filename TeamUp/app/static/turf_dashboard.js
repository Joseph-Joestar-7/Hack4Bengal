...
document.getElementById('add-images-btn').addEventListener('click', () => {
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        // grab the turf_id from a data-attribute in your page:
        const turfId = document
            .getElementById('venue-gallery')
            .getAttribute('data-turf-id');

        try {
            const resp = await fetch(`/turfs/${turfId}/photos`, {
                method: 'POST',
                // If you use CSRF protection (Flask-WTF), you might need to
                // pull the token from a <meta> or hidden input:
            ...
            if (resp.ok) {
                ...
                // Optionally: reload gallery display, or append thumbnails
            } else {
                showErrorMessage(body.error || 'Upload failed');
            }
        } catch (err) {
            console.error(err);
...
function showSuccessMessage(msg) {
    ...
    el.className = 'fixed top-20 right-6 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm';
    el.textContent = msg;
...
function showErrorMessage(msg) {
    const el = document.createElement('div');
    el.className = 'fixed top-20 right-6 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm';
    el.textContent = msg;
    document.body.appendChild(el);
    setTimeout(() => el.remove(), 3000);
}
