...
class TurfManager {
    ...
    async init() {
        // 1) grab real turfs
        try {
            const res = await fetch(window.TURFS_API);
            this.turfData = await res.json();
        } catch (e) {
    ...
    createTurfCard(turf, isSmall = false) {
        const isopen = turf.status==='open'
        const availabilityClass = isopen
            ? 'bg-green-500/80 text-white'
            : 'bg-red-500/80 text-white';
        const availabilityText = isopen ? '✅ Available' : '🔴 Booked';
        const imageHeight = isSmall ? 'h-32' : 'h-32';
            ...
            <div class="turf-card glass-card rounded-3xl overflow-hidden hover:scale-105 transition-all duration-500 group cursor-pointer shadow-xl">
                <div class="relative">
                    ...
                    <div class="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button onclick="window.location.href='/turf/${turf.id}'" class="book-btn w-full bg-emerald-500/90 text-white py-2 rounded-xl font-semibold backdrop-blur-sm hover:bg-emerald-400 transition-colors">
                            Book Now
                        </button>
                    </div>
                </div>
